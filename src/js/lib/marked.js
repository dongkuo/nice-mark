...
(function () {
    ...
    Lexer.prototype.countLine = function (str) {
        var count = 0;
    ...
    Renderer.prototype.code = function (code, lang, escaped, sourceLine) {
        var line = this.options.sourceLine ? 'source-line="' + sourceLine + '"' : '';
    ...
    Renderer.prototype.heading = function (text, level, raw, sourceLine) {
        var line = this.buildSourceLine(sourceLine);
        return '<h' + level + ' id="' + this.options.headerPrefix + raw.toLowerCase().replace(/[^\w]+/g, '-') + '" ' + line + '>' + text + '</h' + level + '>\n';
    };

    Renderer.prototype.hr = function (sourceLine) {
        var line = this.buildSourceLine(sourceLine);
        return this.options.xhtml ? '<hr ' + line + '/>\n' : '<hr ' + line + '>\n';
    };
    ...
    Renderer.prototype.paragraph = function (text, sourceLine) {
        var line = this.buildSourceLine(sourceLine);
        return '<p ' + line + '>' + text + '</p>\n';
    };

    Renderer.prototype.table = function (header, body, sourceLine) {
        var line = this.buildSourceLine(sourceLine);
        return '<table ' + line + '>\n' + '<thead>\n' + header + '</thead>\n' + '<tbody>\n' + body + '</tbody>\n' + '</table>\n';
    };
    ...
    marked.defaults = {
        ...
        xhtml: false,
        sourceLine: false, // 是否在生成的html标签里面添加source-line属性
    };
